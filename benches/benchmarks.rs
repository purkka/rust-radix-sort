...
fn bench_sorts(c: &mut Criterion) {
    ...
    // Plot the results on a logarithmic graph.
    ...
    for (index, vec) in vecs {
        let current = vec;
        let id = i32::pow(10, index);
        group.bench_with_input(BenchmarkId::new("Radix Sort", id), &current,
                                |b, v| b.iter(|| radix_sort(v))).plot_config(plot_config.clone());
        group.bench_with_input(BenchmarkId::new("Sort", id), &current,
